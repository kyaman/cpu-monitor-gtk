	...
	gpointer user_data)
{
	...
	switch(fifoId)
	{
		...
		case 4: {cairo_set_source_rgb (cr, 1.0, 0.0, 1.0); break;}
		case 5: {cairo_set_source_rgb (cr, 0.5, 0.0, 0.0); break;}
		case 6: {cairo_set_source_rgb (cr, 0.0, 0.5, 0.0); break;}
		case 7: {cairo_set_source_rgb (cr, 0.0, 0.0, 0.5); break;}
		case 8: {cairo_set_source_rgb (cr, 0.5, 0.0, 0.5); break;}
		case 9: {cairo_set_source_rgb (cr, 0.0, 0.5, 0.5); break;}
...
int main(int argc, char **argv)
{
	...
	for(i=0; i<g_iCpuCount+1; i++){
		...
		g_signal_connect (G_OBJECT (graph_cpu[i]), "expose-event", G_CALLBACK (cb_expose_event), GINT_TO_POINTER(i));
	}
	...
	// timer
	timer_id = g_timeout_add (1000/RESOLUTION, (GSourceFunc) cb_timer_event, &widgetCpu);
